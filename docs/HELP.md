...
v1.5.0 verwendet den Home-Assistant-Entity-Picker und erlaubt alle `sensor.*`-Entities. Suche nach dem Friendly Name oder der Entity-ID des Leistungs-/Energiesensors der Steckdose. Die Steckdose selbst als `switch.*` ist kein Leistungssensor; benötigt wird der zugehörige `sensor.*`-Messwert.
...
Prüfe, ob das AMS `tray`-Entities besitzt. v1.5.0 erkennt sowohl ältere Unique-IDs wie `..._tray_1` als auch neuere `translation_key: tray` plus `slot`-Attribut.

## Seite springt beim Drucken nach oben

v1.5.0 speichert und restauriert Scrollpositionen um Live-Renderings herum. Falls eine spezielle Custom-View trotzdem springt, bitte Browser, HA-Version und View-Typ im Issue angeben.
